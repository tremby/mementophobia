...
// This file copyright © 2023, P. Lutus and Ryan Fink and Bart Nagel. This file released under the GPL https://www.gnu.org/licenses/gpl-3.0.en.html
// Bart's changelog:
// - Taken from https://github.com/rfink/polyfit.js/blob/ee96691ac127508c40dca7275442912eca59b266/index.js
// - Converted from a CommonJS IIFE to an ES6 module

export default class Polyfit {
	/**
	 * Polyfit
	 * @constructor
	 ...
	 * @returns void
	 */
	gaussJordanDivide(matrix, row, col, numCols) {
	 ...
	 * @returns void
	 */
	gaussJordanEliminate(matrix, row, col, numRows, numCols) {
	...
	standardError(terms) {
		...
		if (n > 2) {
			var a = 0;
			for (var i = 0; i < n; i++) {
				a += Math.pow((this.regress(this.x[i], terms) - this.y[i]), 2);
			}
			r = Math.sqrt(a / (n - 2));
		}
		return r;
